...
library(dplyr)
data = read.table("~/Desktop/Pma_data/Fasta_and_Annotation_files/Intergenic_distance_analysis_genome/All_gene_distances_with_Sus48.bed", com='', sep='\t', header=T)
...
write.table(table,"~/Desktop/Pma_data/Fasta_and_Annotation_files/Intergenic_distance_analysis_genome/distances_with_Sus48.txt",sep="\t", row.names=FALSE)

####To take only significant IDs, the table was exported and ids were selected in excel. These ids were then used to grep TE contents of all genes.
...
plot2 = plot + gghighlight(table$feature == 'effector')
